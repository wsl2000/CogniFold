#!/usr/bin/env bash
# iter29b — minimal additions on top of iter27, after iter29a regression
# evidence showed --tr-* and SESSION_CALENDAR displaced top-K retrievals.
#
# Apples-to-apples 69-qid comparison from iter29a:
#   iter27 87.0% vs iter29a 75.4%  (-11.6pp; MS -25.9 was the killer)
#
# What this run keeps from iter29a (KU +14, SSP +25 evidence):
#   B   Mastra reader rules (KU / PLANNED / RECENT) — still in qa_answer
#   C   inline (meaning DATE) on concept render
#   D'  MS+TR resolver event_date suppression + render strip
#   E   writer 5 Mastra rules (verb/identifier/state/time_phrase/assistant_said)
#   F   --reflector
#   G   reflector lifecycle markers (rendered when F is on)
#   I   verbatim time_phrase + assistant_said render
#   TR-β date_diff_before resolver pattern (always-on in resolver chain)
#
# What this run DROPS from iter29a (each tied to MS / TR / SSU regression):
#   H   SESSION_CALENDAR        — displaced top-K retrievals
#   TR-α TOPIC_TIMELINE         — same
#   TR-ι DAYS_AGO_CHART          — same
#   TR-ξ PARSED_QUESTION         — same
#   TR-κ 2-pass TR reader        — pass2 reversing correct pass1 reasoning
#   TR-δ qa_answer worked examples — stripped from profile (200+ lines bloat)
#   TR-η qa_answer TR CoT          — stripped (forces reader fixate on first value)
#
# Usage:
#   bash scripts/run_iter29b.sh <QID_LIST_FILE> <LABEL> [N_PARALLEL]
...
echo "  reflector: ON   (F+G — kept, helped KU/SSP)"
echo "  tr-*:      OFF  (TR-α/ι/ξ/κ — dropped, hurt MS/SSU/TR)"
echo "  session-calendar: OFF"
...
for ((i=0; i<N_PARALLEL; i++)); do
    ...
    OUTDIR="$BASE/output_i29b_b$((i+1))"
    LOG="logs/iter29b_b$((i+1)).log"
    BATCH_DIRS+=("$OUTDIR")
...
for pid in "${PIDS[@]}"; do
    if ! wait "$pid"; then
        echo "  batch pid=$pid failed (see logs/iter29b_b*.log)"
        FAIL=$((FAIL+1))
    fi
done
...
if final_hyp.exists():
    for line in open(final_hyp):
        try:
            ...
            pass
for batch in sorted(Path("$BASE").glob("output_i29b_b*")):
