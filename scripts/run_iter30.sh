#!/usr/bin/env bash
# iter30 — cleanup branch launcher. Same surface as iter29b but with
# the W3 START extraction pass enabled.
#
# Stack:
#   Reader:    gpt-5.4-mini high
#   Writer:    gpt-5.4-mini low  (W1 + W2 + W3 ON)
#   Rerank:    gpt-5.4-mini low
#   Judge:     gpt-4o
#   Embed:     text-embedding-3-small
#
# iter30 vs iter29b cleanup deltas (see CHANGES.md after run):
#   + W3 START extraction pass (extract_start_events)
#   * D' deep no longer strips inline (meaning DATE) (reader sees it)
#   * Resolver: count_among + order_among disabled (0%-acc on iter27)
#   * Writer prompt: 7→4 rules (dropped verb-precision, SSA, serial)
#   * qa_answer: 267→119 lines (worked examples trimmed)
#   - dead code: build_calendar_block / build_days_ago_chart /
#     build_structured_question_parse / TR-κ 2-pass reader (all flags
#     OFF in iter29b — now physically removed)
#
# Usage:
#   bash scripts/run_iter30.sh <QID_LIST_FILE> <LABEL> [N_PARALLEL]
...
for ((i=0; i<N_PARALLEL; i++)); do
    ...
    OUTDIR="$BASE/output_i30_b$((i+1))"
    LOG="logs/iter30_b$((i+1)).log"
    BATCH_DIRS+=("$OUTDIR")
...
for pid in "${PIDS[@]}"; do
    if ! wait "$pid"; then
        echo "  batch pid=$pid failed (see logs/iter30_b*.log)"
        FAIL=$((FAIL+1))
    fi
done
...
if final_hyp.exists():
    for line in open(final_hyp):
        try:
            ...
            pass
for batch in sorted(Path("$BASE").glob("output_i30_b*")):
