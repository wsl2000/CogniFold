#!/usr/bin/env bash
# iter30b launcher — same as iter30 but W3 OFF.
#
# Drops --extract-start-events because the iter30 N=500 partial showed W3
# contributes -27.6pp MS (UNDERCOUNT pattern: W3-added concepts crowd out
# the actual MS-relevant concepts in top-K retrieval) while only +3.4pp
# TR. Net loss. Reverts to iter29c-equivalent stack but on iter30 code
# (no dead build_*, trimmed prompts, disabled 0%-acc patterns, W3 OFF).
#
