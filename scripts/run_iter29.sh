#!/usr/bin/env bash
# iter29 — Mastra-on-iter27 launcher.
#
# Stack (vs iter27 86.80% N=500 baseline):
#   - Reader  : gpt-5.4-mini reasoning_effort=high   (NTU direct, was OpenRouter in iter27)
#   - Writer  : gpt-5.4-mini reasoning_effort=low    (NTU direct)
#   - Rerank  : gpt-5.4-mini reasoning_effort=low    (NTU direct)
#   - Judge   : gpt-4o                               (NTU direct)
#   - Embed   : text-embedding-3-small               (NTU direct)
#
# New additions (vs iter27):
#   B  Mastra reader rules (KU / PLANNED ACTIONS / MOST RECENT INPUT)
#   C  inline (meaning DATE) on concept lines
#   D' MS+TR resolver event_date suppression (iter27's W2-induced regression)
#   E  writer Mastra rules (verb precision / identifier preservation / state framing)
#   F  Reflector pass (--reflector)
#   G  reflector lifecycle markers in rendered context
#   H  SESSION_CALENDAR top-of-context anchoring table
#   I  verbatim time_phrase + assistant_said in concept.data + render
#   TR-α  topic-focused chronological timeline (--tr-topic-timeline)
#   TR-β  new resolver pattern date_diff_before (always-on, in resolver)
#   TR-δ  worked examples in qa_answer profile
#   TR-ε  count/serial fields in writer prompt
#   TR-η  reader CoT for TR (always-on, in qa_answer profile)
#   TR-ι  DAYS_AGO_CHART for TR questions (--tr-calendar-chart)
#   TR-ν  direction check (always-on, in qa_answer profile)
#   TR-ξ  structured PARSED_QUESTION block (--tr-structured-parse)
#   TR-κ  2-pass TR reader (--tr-two-pass)
#
# Usage:
#   bash scripts/run_iter29.sh <QID_LIST_FILE> <LABEL> [N_PARALLEL]
#
# Example:
#   bash scripts/run_iter29.sh benchmarks/longmemeval/qid_sets/n500.txt \
#        iter29_mastra_on_iter27 10

set -uo pipefail
cd "$(dirname "$0")/.."

[ -f .env ] && set -a && source .env && set +a

QID_LIST_FILE="${1:?need QID_LIST_FILE path}"
LABEL="${2:?need LABEL}"
N_PARALLEL="${3:-10}"

if [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "ERROR: OPENAI_API_KEY missing in .env" >&2
    exit 1
fi

# All roles → NTU direct (sk-proj key). No OpenRouter, no commonstack.
export WRITER_API_KEY="$OPENAI_API_KEY"
export WRITER_BASE_URL=""
export READER_API_KEY="$OPENAI_API_KEY"
export READER_BASE_URL=""
export JUDGE_API_KEY="$OPENAI_API_KEY"
export JUDGE_BASE_URL=""
export EMBEDDING_API_KEY="$OPENAI_API_KEY"
export EMBEDDING_BASE_URL=""
export WRITER_REASONING_EFFORT="low"
export REFLECTOR_REASONING_EFFORT="low"

# Keep the global OPENAI_* pointing at NTU direct (fallback).
unset OPENAI_BASE_URL
unset OPENAI_ORGANIZATION

BASE="benchmarks/longmemeval"
FINAL_DIR="$BASE/runs/$LABEL"
mkdir -p "$FINAL_DIR" logs

echo "iter29 run: $LABEL"
echo "  qid file: $QID_LIST_FILE"
echo "  parallel: $N_PARALLEL"
echo "  reader:   NTU direct → gpt-5.4-mini high"
echo "  writer:   NTU direct → gpt-5.4-mini low (W1 + W2 ON)"
echo "  rerank:   NTU direct → gpt-5.4-mini low"
echo "  judge:    NTU direct → gpt-4o"
echo "  embed:    NTU direct → text-embedding-3-small"
echo "  output:   $FINAL_DIR"

# Compute TODO qids (resume support).
TODO_FILE=$(mktemp)
trap "rm -f $TODO_FILE" EXIT
.venv/bin/python <<PY > "$TODO_FILE"
import json, sys
from pathlib import Path
text = Path("$QID_LIST_FILE").read_text().strip()
raw = [t.strip() for chunk in text.split("\n") for t in chunk.split(",")]
target_qids = [q for q in raw if q]
done = set()
final_hyp = Path("$FINAL_DIR/hypothesis.jsonl")
if final_hyp.exists():
    for line in open(final_hyp):
        try:
            done.add(json.loads(line)["question_id"])
        except Exception:
            pass
todo = [q for q in target_qids if q not in done]
sys.stderr.write(f"target={len(target_qids)} done={len(done)} todo={len(todo)}\n")
for q in todo:
    print(q)
PY

N_TODO=$(wc -l < "$TODO_FILE")
if [ "$N_TODO" -eq 0 ]; then
    echo "Nothing to do — all qids already in $FINAL_DIR/hypothesis.jsonl"
    exit 0
fi
if [ "$N_TODO" -lt "$N_PARALLEL" ]; then
    N_PARALLEL="$N_TODO"
fi
echo "Launching $N_PARALLEL parallel batches over $N_TODO todo qids"

CHUNK=$(( N_TODO / N_PARALLEL ))
REM=$(( N_TODO - CHUNK * N_PARALLEL ))

PIDS=()
BATCH_DIRS=()
START=1
for ((i=0; i<N_PARALLEL; i++)); do
    LIMIT=$CHUNK
    if [ "$i" -lt "$REM" ]; then LIMIT=$((CHUNK + 1)); fi
    END=$(( START + LIMIT - 1 ))
    IDS_CSV=$(sed -n "${START},${END}p" "$TODO_FILE" | paste -sd, -)
    OUTDIR="$BASE/output_i29_b$((i+1))"
    LOG="logs/iter29_b$((i+1)).log"
    BATCH_DIRS+=("$OUTDIR")
    echo "  batch $((i+1)): $LIMIT qid → $OUTDIR (log: $LOG)"
    rm -rf "$OUTDIR" && mkdir -p "$OUTDIR"
    nohup .venv/bin/python -u -m benchmarks.longmemeval.run_eval \
        --model openai:gpt-5.4-mini \
        --writer-model openai:gpt-5.4-mini \
        --judge-model openai:gpt-4o \
        --embedding openai:text-embedding-3-small \
        --writer-reasoning-effort low \
        --extract-typed-attributes \
        --resolve-event-dates \
        --reflector \
        --tr-topic-timeline \
        --tr-calendar-chart \
        --tr-two-pass \
        --tr-structured-parse \
        --symbolic-resolver --symbolic-temporal --symbolic-bypass \
        --llm-rerank --rerank-model openai:gpt-5.4-mini \
        --rerank-reasoning-effort low --rerank-pool 100 \
        --agg-max-context-chars 15000 \
        --question-ids "$IDS_CSV" \
        --output-dir "$OUTDIR" \
        --batch-mode --llm-eval \
        > "$LOG" 2>&1 &
    PIDS+=($!)
    START=$(( END + 1 ))
done

echo "PIDs: ${PIDS[*]}"
echo "Waiting..."
FAIL=0
for pid in "${PIDS[@]}"; do
    if ! wait "$pid"; then
        echo "  batch pid=$pid failed (see logs/iter29_b*.log)"
        FAIL=$((FAIL+1))
    fi
done

# Merge batch results into final.
.venv/bin/python <<PY
import json
from collections import Counter
from pathlib import Path

final_hyp = Path("$FINAL_DIR/hypothesis.jsonl")
records = {}
if final_hyp.exists():
    for line in open(final_hyp):
        try:
            r = json.loads(line); records[r["question_id"]] = r
        except Exception:
            pass
for batch in sorted(Path("$BASE").glob("output_i29_b*")):
    hp = batch / "hypothesis.jsonl"
    if not hp.exists():
        continue
    for line in open(hp):
        try:
            r = json.loads(line); records[r["question_id"]] = r
        except Exception:
            pass

with open(final_hyp, "w") as f:
    for r in records.values():
        f.write(json.dumps(r) + "\n")

c = Counter(r.get("verdict") for r in records.values())
total = len(records)
strict = c["CORRECT"]/total*100 if total else 0
partial = (c["CORRECT"] + 0.5*c["PARTIAL"])/total*100 if total else 0
with open("$FINAL_DIR/metrics.json", "w") as f:
    json.dump({
        "correct": c["CORRECT"], "partial": c["PARTIAL"],
        "incorrect": c["INCORRECT"], "error": c.get("ERROR", 0),
        "total": total,
        "score_strict": strict, "score_partial": partial,
    }, f, indent=2)
print(f"merged: {total} results — {c['CORRECT']}/{total} = {strict:.2f}% strict, {partial:.2f}% partial")
PY

if [ "$FAIL" -eq 0 ]; then
    for d in "${BATCH_DIRS[@]}"; do rm -rf "$d"; done
    echo "Done. Result at $FINAL_DIR/{hypothesis,metrics}.json"
else
    echo "WARNING: $FAIL batches failed; per-batch dirs preserved." >&2
    exit 1
fi
